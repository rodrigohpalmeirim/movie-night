<script>
    ...
    export let video, activeTextTrack, open=false, toggleOpen,sendSubtitles,subtitles;
    let lastTextTrack;
...
{#if video}
    <div class="relative w-8 group transition-all">
        <div class="absolute -right-4 bottom-10 {open? "w-32 h-48 bg-slate-800":"w-8 h-8"} rounded-2xl transition-all overflow-hidden {open?"opacity-100":"opacity-0"}">
